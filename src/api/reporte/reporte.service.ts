...
export class ReporteService {
  ...
  async obtenerDetallesPorRango(entidad: string, inicio: string, fin: string) {
    ...
    if(entidad === 'ventas'){
      ...
      for (const venta of ventas) {
        const encontrado = await this.ventaDetalleModel.find({ venta: venta._id }).populate('producto').populate('variedad').populate('cliente');
      ...
      for (const ingreso of ingresos) {
        const encontrado = await this.ingresoDetalleModel.find({ ingreso: ingreso._id }).populate('producto_variedad').populate('producto').populate('almacen');
        detalles.push(...encontrado);
      }
      ...
      for (const egreso of egresos) {
        const encontrado = await this.ingresoDetalleModel.find({ ingreso: egreso._id }).populate('producto_variedad').populate('producto').populate('almacen');
        detalles.push(...encontrado);
      }
      ...
      for (const compra of compras) {
        const encontrado = await this.ingresoDetalleModel.find({ ingreso: compra._id }).populate('producto_variedad').populate('producto').populate('almacen');
        detalles.push(...encontrado);
      }
